...
with st.sidebar.expander("📐 4. Layout", expanded=True):
    if modo == "QR Code":
        tamanho_qr = st.slider("Tamanho QR (px)",50, max_w, 450,10, key="qr_tam_qr")
        tam_txt    = st.slider("Tamanho Nº (pt)",10, max(10,max_h//2),150,5, key="qr_tam_txt")
        cor_txt    = st.color_picker("Cor Nº","#000000", key="qr_cor")
        c1,c2=st.columns(2)
        qr_x  =c1.number_input("QR X", 0,max_w,540,5,key="qr_x")
        qr_y  =c2.number_input("QR Y", 0,max_h,1035,5,key="qr_y")
        txt_x =c1.number_input("Nº X", 0,max_w,533,5,key="qr_txt_x")
        txt_y =c2.number_input("Nº Y", 0,max_h,1445,5,key="qr_txt_y")
        ...
        larg_bar=st.slider("Largura (px)",100,max_w,570,10,key="bc_largura")
        alt_bar =st.slider("Altura (px)", 50,max(50,max_h//2),215,5,key="bc_altura")
        tam_txt =st.slider("Tamanho Nº (pt)",10,max(10,max_h//2),142,2,key="bc_tam_txt")
        cor_txt =st.color_picker("Cor Nº","#FFFFFF",key="bc_cor")
        c1,c2=st.columns(2)
        bar_x=c1.number_input("Barra X",0,max_w,535,5,key="bc_x")
        bar_y=c2.number_input("Barra Y",0,max_h,600,5,key="bc_y")
        txt_x=c1.number_input("Nº X",  0,max_w,535,5,key="bc_txt_x")
        txt_y=c2.number_input("Nº Y",  0,max_h,845,5,key="bc_txt_y")
